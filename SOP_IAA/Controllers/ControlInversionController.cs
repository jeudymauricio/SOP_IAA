    ...
    public class ControlInversionController : Controller
    {
        ...
        public ActionResult Periodo(int? idContrato, DateTime fecha)
        {
            ...
            foreach(var item in contrato.contratoItem){
                ...
                // Se seleccionan las ordenes de modificación validas para la fecha
                var om = item.oMCI.Where(o => (o.ordenModificacion.fecha.Month <= fecha.Month && o.ordenModificacion.fecha.Year <= fecha.Year));
                ...
                var detalleItem = new Tuple<string, string, string, decimal, decimal, decimal>(item.item.codigoItem, item.item.descripcion, item.item.unidadMedida, cantidadAutorizada, cantidadLaborada, decimal.Round(cantidadAutorizada - cantidadLaborada, 3));
